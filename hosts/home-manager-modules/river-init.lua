...
local startup_commands = {
    ...
    {'swayidle', 'timeout 300 "swaylock"'}, {
        wl_script_dir .. '/launch_waybar', '-c',
        '~/.config/waybar/river/config', '-s',
        '~/.config/waybar/river/style.css'
    }, {wl_script_dir .. '/clipboard', '--start'},
    wl_script_dir .. '/launch_pipewire', wl_script_dir .. '/launch_mpd',
    wl_script_dir .. '/launch_notify', wl_script_dir .. '/wlwpp',
...
local window_rules = {
    ['float-filter-add'] = {
        ['app-id'] = {
            'float', 'popup', 'swappy', 'pinentry-qt', 'pavucontrol-qt'
        },
        ['title'] = {
            'Picture-in-Picture'
            -- 'About *',
...
-- key: string
local modes = {{name = 'passthrough', mod = 'Super', key = 'F11'}}
...
local mappings = {
    ...
    map = {
        normal = {
            -- Terminal emulators
            {mod = 'Super', key = 'Return', command = {'spawn', 'foot'}},
            {
                mod = {'Super', 'Shift'},
                key = 'Return',
            ...
            }, -- Web search
            {
                ...
                key = 'S',
                command = {
                    'spawn', string.format([['%s/searchmenu']], wl_script_dir)
                }
            }, -- Quickly open offline manga
            {
                ...
                key = 'M',
                command = {
                    'spawn', string.format([['%s/mangamenu']], wl_script_dir)
                }
            }, -- Browser bookmarks (buku)
            {
                ...
                key = 'B',
                command = {
                    'spawn', string.format([['%s/bukumenu']], wl_script_dir)
                }
            }, -- Open current MPD playlist
            {
                ...
                key = 'U',
                command = {
                    'spawn', string.format([['%s/musicmenu']], wl_script_dir)
                }
            ...
            }, -- Clipboard management with clipman
            {
                ...
                command = {
                    'spawn',
                    string.format([['%s/clipboard --rofi']], wl_script_dir)
                }
            }, {
                mod = {'Super', 'Alt'},
                key = 'C',
                command = {
                    'spawn',
                    string.format([['%s/clipboard --clear']], wl_script_dir)
                }
            }, {
                mod = 'Super',
                key = 'C',
                command = {
                    'spawn',
                    string.format([['%s/clipboard --rofi-clear']], wl_script_dir)
                }
            }, -- Dismiss notifications
            {
                ...
                command = {
                    'spawn',
                    string.format([['%s/dismiss_notify']], wl_script_dir)
                }
            }, -- Launch Emacs
            {
                mod = {'Super', 'Alt'},
            ...
            }, -- Taking screenshots
            {
                ...
                command = {
                    'spawn',
                    string.format([['%s/screenshot --full']], wl_script_dir)
                }
            }, {
                mod = 'None',
                key = 'Print',
                command = {
                    'spawn',
                    string.format([['%s/screenshot --region']], wl_script_dir)
                }
            }, {
                mod = 'Alt',
                key = 'Print',
                command = {
                    'spawn',
                    string.format([['%s/screenshot --region-optional']],
                                  wl_script_dir)
                }
            }, {
                mod = 'Control',
                ...
                command = {
                    'spawn',
                    string.format([['%s/screenshot --full-optional']],
                                  wl_script_dir)
                }
            }, {
                mod = {'Super', 'Alt'},
                ...
                command = {
                    'spawn',
                    string.format([['%s/screenshot --region-copy']],
                                  wl_script_dir)
                }
            }, {
                mod = {'Super', 'Control'},
                ...
                command = {
                    'spawn',
                    string.format([['%s/screenshot --full-copy']], wl_script_dir)
                }
            }, -- Super+Q to close the focused view
            {mod = 'Super', key = 'Q', command = 'close'},
            -- Super+Shift+Q to exit river (requires 'swaynag' program from sway)
            {
                ...
                command = {
                    'spawn',
                    [['swaynag -t warning -m "Exit river?" -b "Yes" "riverctl exit"']]
                }
            ...
            {mod = 'Super', key = 'J', command = {'focus-view', 'previous'}},
            {mod = 'Super', key = 'K', command = {'focus-view', 'next'}},
            ...
            {mod = {'Super', 'Shift'}, key = 'K', command = {'swap', 'next'}},
            -- Super+{P,N} to focus next/previous output
            {mod = 'Super', key = 'P', command = {'focus-output', 'previous'}},
            {mod = 'Super', key = 'N', command = {'focus-output', 'next'}},
            ...
            {mod = {'Super', 'Alt'}, key = 'H', command = {'move', 'left', 100}},
            {mod = {'Super', 'Alt'}, key = 'J', command = {'move', 'down', 100}},
            {mod = {'Super', 'Alt'}, key = 'K', command = {'move', 'up', 100}},
            {
                mod = {'Super', 'Alt'},
                key = 'L',
            ...
            {mod = 'Super', key = 'Space', command = 'toggle-float'},
            -- Super+F to toggle fullscreen
            {mod = 'Super', key = 'F', command = 'toggle-fullscreen'}
        },
        locked = {
            ...
            }, -- Control pulseaudio volume
            {
                ...
                command = {
                    'spawn',
                    string.format([['%s/volumecontrol --increase']],
                                  wl_script_dir)
                },
                opt = 'repeat'
            }, {
                ...
                command = {
                    'spawn',
                    string.format([['%s/volumecontrol --decrease']],
                                  wl_script_dir)
                },
                opt = 'repeat'
            }, {
                ...
                command = {
                    'spawn',
                    string.format([['%s/volumecontrol --toggle']], wl_script_dir)
                }
            }, {
                mod = 'None',
                ...
                command = {
                    'spawn',
                    string.format([['%s/volumecontrol --toggle-source']],
                                  wl_script_dir)
                }
            ...
            }, -- Control screen backlight brightness
            {
                ...
                command = {
                    'spawn',
                    string.format([['%s/brightness up']], wl_script_dir)
                },
                opt = 'repeat'
            }, {
                mod = 'None',
                ...
                command = {
                    'spawn',
                    string.format([['%s/brightness down']], wl_script_dir)
                },
                opt = 'repeat'
            }
        }
    ...
    ['map-pointer'] = {
        normal = {
            ...
            {mod = 'Super', key = 'BTN_LEFT', command = 'move-view'},
            -- Super + Right Mouse Button to resize views
            {mod = 'Super', key = 'BTN_RIGHT', command = 'resize-view'}
        }
    }
}
...
local function tag_mappings()
    for i = 1, 9 do
        ...
        os.execute(string.format(
                       'riverctl map normal Super %s set-focused-tags %s', i,
                       tag_num))

        -- Super+Shift+[1-9] to tag focused view with tag [0-8]
        os.execute(string.format(
                       'riverctl map normal Super+Shift %s set-view-tags %s', i,
                       tag_num))

        -- Super+Control+[1-9] to toggle focus of tag [0-8]
        os.execute(string.format(
                       'riverctl map normal Super+Control %s toggle-focused-tags %s',
                       i, tag_num))

        -- Super+Alt+[1-9] to toggle tag [0-8] of focused view
        os.execute(string.format(
                       'riverctl map normal Super+Alt %s toggle-view-tags %s',
                       i, tag_num))
    end

    -- river has a total of 32 tags
    local all_tags = (1 << 32) - 1
    os.execute(string.format('riverctl map normal Super 0 set-focused-tags %s',
                             all_tags))
    os.execute(string.format(
                   'riverctl map normal Super+Shift 0 set-view-tags %s',
                   all_tags))
end
...
for output, options in pairs(outputs) do
    ...
    if options.preferred then randr_cmd = randr_cmd .. ' --preferred' end
end
...
for _, mode in ipairs(modes) do
    ...
    os.execute(string.format('riverctl map normal %s %s enter-mode %s',
                             modifiers, mode.key, mode_name))
    os.execute(string.format('riverctl map %s %s %s enter-mode normal',
                             mode_name, modifiers, mode.key))
end
...
for map_type, tbl in pairs(mappings) do
    for mode, value in pairs(tbl) do
        for _, binding in ipairs(value) do
            ...
            os.execute(string.format('riverctl %s %s %s %s %s %s', map_type,
                                     opt, mode, modifiers, binding.key, cmd))

            -- Duplicate mappings of mode 'locked' for mode 'normal'
            if mode == 'locked' then
                os.execute(string.format('riverctl %s %s normal %s %s %s',
                                         map_type, opt, modifiers, binding.key,
                                         cmd))
            end
        end
    end
...
unistd.execp('rivertile', {
    '-view-padding', 5, '-outer-padding', 5, '-main-location', 'left',
    '-main-count', 1, '-main-ratio', 0.54
})
